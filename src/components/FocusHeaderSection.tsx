          ...
          {sortedUrgent.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
              {sortedUrgent.map(task => {
                ...
                if (task.deadline && !task.isDone) {
                  ...
                  if (diff < 0) {
                    deadlineBadge = <span className="text-[9px] bg-red-100 text-red-700 px-1 py-0.2 rounded border border-red-200 font-mono font-bold">期限切</span>;
                  } else if (diff <= deadlineThresholdDays * 86400000) {
                    const days = Math.ceil(diff / 86400000);
                    deadlineBadge = <span className="text-[9px] bg-amber-100 text-amber-800 px-1 py-0.2 rounded border border-amber-200 font-mono font-bold">{days <= 0 ? '本日' : `${days}日後`}</span>;
                  }
                }
            ...
            <div className="py-4 px-3 flex items-center justify-between bg-white rounded-lg border border-dashed border-red-200 text-xs text-slate-400">
              <div className="flex items-center gap-2">
                ...
                <span>現在Focusスロットに設定されたタスクはありません（最大 {urgentLimit} 件）</span>
              </div>
              <button
                onClick={onOpenDailyPick}
